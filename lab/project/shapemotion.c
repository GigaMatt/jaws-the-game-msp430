...
AbRect rectangle_size_7 = {abRectGetBounds, abSlicedRectCheck, {15, 7}};    /**< 7x7 human rectangle */
  ...
  for (; ml; ml = ml->next)
  {
    ...
    for (axis = 0; axis < 2; axis++)
    {
          ...
          (shapeBoundary.botRight.axes[axis] > fence->botRight.axes[axis]))
      {
        ...
        if (velocity < 0)
        {
          drawString5x7(9, 35, "JAWS: IT'S A BAD     ", COLOR_RED, COLOR_BLACK); //prev 5x7 20, 35
          buzzer_set_period(1000);
          newPos.axes[axis] += (2 * velocity);
        }
        if (velocity > 0)
        {
          drawString5x7(9, 35, "WEEK TO BE A SEAL      ", COLOR_RED, COLOR_BLACK); //prev 5x7 20, 35          
        }
      } /**< if outside of fence */
    }   /**< for axis */
